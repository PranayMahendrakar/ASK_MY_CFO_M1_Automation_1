...
Flow:
  1. extract_pages(pdf)        → *_BS_PL.pdf   (only BS/PL pages)
    2. extract_tables(bs_pl_pdf) → *_extracted.xlsx (structured rows)
      ...
      API key is read from OPENAI_API_KEY environment variable (Render secret).
      """
import os
import sys
import json
...
# Read API key from environment (Render secret) — never from user input
...
class PipelineJob:
      def __init__(self, job_id, pdf_paths, skip_stage1):
                self.job_id      = job_id
                self.pdf_paths   = pdf_paths
                self.skip_stage1 = skip_stage1
                self.skip_stage3 = not bool(OPENAI_API_KEY)
                self.status      = "queued"
                self.stage       = 0
                self.stage_label = ""
                self.progress    = 0
      ...
      def log(self, msg, level="info"):
                ...
                self._push({"type": "log", "data": entry,
        ...
        for fi, pdf_path in enumerate(job.pdf_paths):
            ...
            try:
                ...
                if expected.exists():
                    ...
                    for f in sorted(out_dir.glob("*_extracted.xlsx"),
            ...
            # ═══════════════════════════════════════
            if job.skip_stage3:
                              job.set_stage(3, "Skipped — OPENAI_API_KEY not set", base_pct + int(span * 0.95))
                              job.log("Stage 3 skipped (set OPENAI_API_KEY env var on Render to enable)")
                              continue
...
def health():
      return jsonify({
                ...
                "stage3_available": bool(OPENAI_API_KEY),
      })
...
def upload():
    ...
    job = PipelineJob(job_id, pdf_paths, skip_stage1)
    jobs[job_id] = job
